...
pub struct SoxResampler {
    ...
    handle_id: ::std::sync::OnceLock<crate::FfiHandleId>,
}
...
impl SoxResampler {
    ...
    pub fn new(
        ...
        let obj = Self { inner: Mutex::new(inner), handle_id: ::std::sync::OnceLock::new() };
...
/// SoX recipe numbers are not contiguous, so they are mapped here rather than
/// carried as enum discriminants. `SOXR_HQ` and `SOXR_VHQ` are aliases that
/// bindgen does not emit, so their targets are named directly.
impl From<SoxQualityRecipe> for c_ulong {
    fn from(value: SoxQualityRecipe) -> Self {
        let recipe = match value {
            SoxQualityRecipe::Quick => soxr_sys::SOXR_QQ,
            SoxQualityRecipe::Low => soxr_sys::SOXR_LQ,
            SoxQualityRecipe::Medium => soxr_sys::SOXR_MQ,
            SoxQualityRecipe::High => soxr_sys::SOXR_20_BITQ,
            SoxQualityRecipe::VeryHigh => soxr_sys::SOXR_28_BITQ,
        };
        recipe as c_ulong
    }
}
...
#[cfg(test)]
mod quality_recipe_tests {
    use super::*;

    #[test]
    fn recipes_map_to_sox_constants() {
        assert_eq!(c_ulong::from(SoxQualityRecipe::Quick), 0);
        assert_eq!(c_ulong::from(SoxQualityRecipe::Low), 1);
        assert_eq!(c_ulong::from(SoxQualityRecipe::Medium), 2);
        assert_eq!(c_ulong::from(SoxQualityRecipe::High), 4);
        assert_eq!(c_ulong::from(SoxQualityRecipe::VeryHigh), 6);
    }
}
