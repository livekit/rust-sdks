use livekit_token_source::{
    TokenSourceConfigurable, TokenSourceFetchOptions, TokenSourceFixed, TokenSourceResponse, TokenSourceResult
};
...
async fn main() {
    ...
    let literal = livekit_token_source::literal(
    ...
    let file_source = FileTokenSource {
        path: concat!(env!("CARGO_MANIFEST_DIR"), "/token.txt").into(),
    };
    ...
    let development_token_server = livekit_token_source::development_token_server(sandbox_id.clone());
