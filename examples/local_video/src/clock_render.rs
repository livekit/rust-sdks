//! Reusable low-latency millisecond clock renderer.
//!
//! Draws a seven-segment `HH:MM:SS.mmm` clock with a millisecond grid into a wgpu
//! render pass. The renderer is shared by the standalone `clock` binary and by the
//! publisher/subscriber preview windows (rendered below the video when `--clock` is set).
...
/// Paints the current wall-clock time into the callback's allocated rect.
///
/// `viewport_px` is the size of the target rect in physical pixels; it drives the
/// shader's aspect ratio so the clock stays undistorted when it only covers part of
/// the window (e.g. the band below the video). Add it to a painter via
/// [`egui_wgpu::Callback::new_paint_callback`].
pub(crate) struct ClockPaintCallback {
    pub(crate) viewport_px: [u32; 2],
}

impl ClockPaintCallback {
    /// Build a callback sized to `rect`, converting points to physical pixels.
    pub(crate) fn for_rect(ctx: &egui::Context, rect: egui::Rect) -> Self {
...
impl CallbackTrait for ClockPaintCallback {
    fn prepare(
        ...
        device: &wgpu::Device,
        queue: &wgpu::Queue,
        _screen_descriptor: &egui_wgpu_backend::ScreenDescriptor,
        ...
        let uniform = ClockUniform::new(self.viewport_px, current_clock_chars());
