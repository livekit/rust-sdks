...
- `--min-bitrate <bps>`: Min video bitrate for the main (highest) layer in bits per second. In libcamera throughput mode, the publisher defaults this to the main max bitrate so WebRTC does not silently starve the hardware encoder.
...
- On Raspberry Pi, `--source libcamera` is tuned first for H.264 at 1280x720@30 and can negotiate 1920x1080 when the Pi encoder/bitrate can keep up. The publisher disables H.265 and simulcast for this path because the V4L2 M2M H.264 encoder is a single-stream encoder. In the periodic WebRTC stats, expect `encoder: V4L2 H264 Encoder`; startup logs should show `LibCameraCapture: capture path: libcamera DMABUF native frames`, `Publisher: capture frame path: native DMABUF frames`, and `V4L2: encoder input path confirmed: DMABUF zero-copy import`. If you instead see `CPU I420 copy into V4L2 MMAP`, the encoder is not on the zero-copy path. For a normal adaptive WebRTC send, pass `--degradation-preference maintain-resolution`; the default libcamera `auto` mode disables WebRTC adaptation to make hardware encoder fps visible.
