...
```
 ...
 # force software decode, useful when diagnosing platform decoder issues
 cargo run -p local_video -F desktop --bin subscriber -- \
   --room-name demo \
   --identity viewer-1 \
...
- `--video-decoder <default|software>`: Select the video decoder preference. `software` sets `LIVEKIT_VIDEO_DECODER=software` before connecting, which skips platform hardware decoder probes.
