...
- On Linux, NVDEC H.264/H.265 frames automatically use a CUDA-to-Vulkan render path when WGPU selects the same NVIDIA GPU and the required external-memory and semaphore extensions are available. Otherwise the subscriber falls back to CPU I420 upload; setting `LK_DISABLE_NVDEC=1` or selecting a non-Vulkan WGPU backend exercises that fallback.
- The Linux NVDEC decoder uses decode-order output to avoid a frame of display reordering. This matches the I/P-only real-time streams produced by the local video publishers; sources containing B-frames require display-order decoding instead.
