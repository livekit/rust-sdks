...
- Keep only a weak runtime reference in the engine after introducing runtime-scoped session guards, so detached engine tasks cannot retain a closed peer connection factory across SDK lifecycles.
