...
- Serialize runtime lifecycles: `LkRuntime::instance()` now waits (bounded) for a previous runtime's teardown to fully complete before constructing a new peer connection factory/ADM, and `FfiServer::dispose` waits for the runtime to finish tearing down before returning. This closes a race where factory/ADM/transport destruction from one initialize/shutdown cycle could overlap the next cycle's startup. Adds `LkRuntime::wait_for_teardown()`.
