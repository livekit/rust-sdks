...
#define CHECK(cond, fmt, ...) do { \
    if(!(cond)) { \
      fprintf(stderr, "implib-gen: libv4l2.so.0: " fmt "\n", ##__VA_ARGS__); \
...
static int load_library(void) {
  ...
  lib_handle = lk_jetson_dlopen("libv4l2.so.0");
  CHECK(lib_handle, "failed to load library 'libv4l2.so.0' via callback 'lk_jetson_dlopen'");
#else
  lib_handle = dlopen("libv4l2.so.0", RTLD_LAZY | RTLD_GLOBAL);
  CHECK(lib_handle, "failed to load library 'libv4l2.so.0' via dlopen: %s", dlerror());
#endif
...
extern void *_libv4l2_so_tramp_table[];

// Can be sped up by manually parsing library symtab...
void *_libv4l2_so_tramp_resolve(size_t i) {
  assert(i < SYM_COUNT);
...
#   endif
  ...
  if (publish) {
    ...
    (void)__sync_val_compare_and_swap(&_libv4l2_so_tramp_table[i], 0, addr);
  }
...
void _libv4l2_so_tramp_resolve_all(void) {
  size_t i;
  for(i = 0; i < SYM_COUNT; ++i)
    _libv4l2_so_tramp_resolve(i);
}

// Allows user to specify manually loaded implementation library.
void _libv4l2_so_tramp_set_handle(void *handle) {
...
void _libv4l2_so_tramp_reset(void) {
  // TODO: call unload_lib ?
  memset(_libv4l2_so_tramp_table, 0, SYM_COUNT * sizeof(_libv4l2_so_tramp_table[0]));
  lib_handle = 0;
  dlopened = 0;
}
