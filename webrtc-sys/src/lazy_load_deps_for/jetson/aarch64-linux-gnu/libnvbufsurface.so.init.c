...
#define CHECK(cond, fmt, ...) do { \
    if(!(cond)) { \
      fprintf(stderr, "implib-gen: libnvbufsurface.so.1.0.0: " fmt "\n", ##__VA_ARGS__); \
...
static int load_library(void) {
  ...
  extern void *lk_jetson_dlopen(const char *lib_name);
  lib_handle = lk_jetson_dlopen("libnvbufsurface.so.1.0.0");
  CHECK(lib_handle, "failed to load library 'libnvbufsurface.so.1.0.0' via callback 'lk_jetson_dlopen'");
#else
  lib_handle = dlopen("libnvbufsurface.so.1.0.0", RTLD_LAZY | RTLD_GLOBAL);
  CHECK(lib_handle, "failed to load library 'libnvbufsurface.so.1.0.0' via dlopen: %s", dlerror());
#endif
...
extern void *_libnvbufsurface_so_tramp_table[];

// Can be sped up by manually parsing library symtab...
void *_libnvbufsurface_so_tramp_resolve(size_t i) {
  assert(i < SYM_COUNT);
...
#   endif
  ...
  if (publish) {
    ...
    (void)__sync_val_compare_and_swap(&_libnvbufsurface_so_tramp_table[i], 0, addr);
  }
...
void _libnvbufsurface_so_tramp_resolve_all(void) {
  size_t i;
  for(i = 0; i < SYM_COUNT; ++i)
    _libnvbufsurface_so_tramp_resolve(i);
}

// Allows user to specify manually loaded implementation library.
void _libnvbufsurface_so_tramp_set_handle(void *handle) {
...
void _libnvbufsurface_so_tramp_reset(void) {
  // TODO: call unload_lib ?
  memset(_libnvbufsurface_so_tramp_table, 0, SYM_COUNT * sizeof(_libnvbufsurface_so_tramp_table[0]));
  lib_handle = 0;
  dlopened = 0;
}
