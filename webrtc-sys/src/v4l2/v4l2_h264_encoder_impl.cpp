...
bool V4L2H264EncoderImpl::InitializeV4L2Device() {
  RTC_LOG(LS_INFO) << "V4L2 H264 Encoder: Opening device " << device_path_;
...
void V4L2H264EncoderImpl::CleanupV4L2Device() {
  if (device_fd_ >= 0) {
    DeallocateBuffers();
    close(device_fd_);
    device_fd_ = -1;
...
bool V4L2H264EncoderImpl::AllocateInputBuffers() {
  struct v4l2_requestbuffers req;
  memset(&req, 0, sizeof(req));
  req.count = 6;  // Request 6 buffers for input
  ...
  for (uint32_t i = 0; i < num_input_buffers_; i++) {
    ...
    buf.index = i;
    buf.m.planes = planes;
    buf.length = 3;  // For I420: Y, U, V planes
    ...
    // Map only the first plane for now (we'll handle multi-plane later)
...
bool V4L2H264EncoderImpl::AllocateOutputBuffers() {
  struct v4l2_requestbuffers req;
  memset(&req, 0, sizeof(req));
  req.count = 6;  // Request 6 buffers for output
...
void V4L2H264EncoderImpl::DeallocateBuffers() {
...
int32_t V4L2H264EncoderImpl::InitEncode(
  ...
  // Code expects simulcastStream resolutions to be correct
  ...
  // Set capture format (encoded H264 output from encoder) FIRST
  ...
  capture_format_.fmt.pix_mp.pixelformat = V4L2_PIX_FMT_H264;
  ...
  if (ioctl(device_fd_, VIDIOC_S_CTRL, &ctrl) < 0) {
    RTC_LOG(LS_WARNING) << "Failed to set H264 profile, continuing anyway";
  }

  // Set I-frame interval
  ctrl.id = V4L2_CID_MPEG_VIDEO_H264_I_PERIOD;
  ctrl.value = configuration_.key_frame_interval;
  if (ioctl(device_fd_, VIDIOC_S_CTRL, &ctrl) < 0) {
    RTC_LOG(LS_WARNING) << "Failed to set I-frame interval, continuing anyway";
  }
  ...
  RTC_LOG(LS_INFO) << "V4L2 H264 encoder initialized: "
...
bool V4L2H264EncoderImpl::EncodeFrame(const VideoFrame& frame, bool is_keyframe) {
  ...
  if (ioctl(device_fd_, VIDIOC_DQBUF, &buf) < 0) {
    if (errno == EAGAIN) {
      ...
      RTC_LOG(LS_VERBOSE) << "V4L2 H264: no input buffer available yet (EAGAIN)";
      return true;
    }
  ...
  // For now, we'll use a simple approach - copy Y, U, V planes
  uint8_t* buffer_ptr = static_cast<uint8_t*>(input_buffers_[buf.index]);
  
  // Calculate sizes and offsets for I420
...
int32_t V4L2H264EncoderImpl::ProcessEncodedFrame(
...
VideoEncoder::EncoderInfo V4L2H264EncoderImpl::GetEncoderInfo() const {
  ...
  info.implementation_name = "V4L2 H264 Encoder (Jetson)";
