...
std::vector<uint8_t> BuildAv1MetadataObu(
    webrtc::ArrayView<const uint8_t> trailer) {
...
size_t FindAv1MetadataInsertOffset(webrtc::ArrayView<const uint8_t> data) {
  size_t pos = 0;
...
std::optional<PacketTrailerMetadata> ExtractAv1Trailer(
...
void PacketTrailerTransformer::TransformSend(
  ...
  const bool is_av1 = IsAv1Frame(*frame);
...
std::vector<uint8_t> PacketTrailerTransformer::AppendTrailer(
  ...
  if (is_av1) {
    std::vector<uint8_t> obu = BuildAv1MetadataObu(trailer);
    if (data.empty()) {
      return obu;
    }
    const size_t insert_offset = FindAv1MetadataInsertOffset(data);
...
std::optional<PacketTrailerMetadata> PacketTrailerTransformer::ExtractTrailer(
    ...
    bool is_av1) {
  if (is_av1) {
    return ExtractAv1Trailer(data, out_data);
  }
