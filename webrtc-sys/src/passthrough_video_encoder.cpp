...
int32_t PassthroughVideoEncoder::InitEncode(
  ...
  sending_ = true;
  RTC_LOG(LS_INFO) << "PassthroughVideoEncoder::InitEncode " << codec_.width
                   << "x" << codec_.height;
...
int32_t PassthroughVideoEncoder::Encode(
  ...
  // Surface PLI/FIR keyframe requests back to the Rust caller (best-effort).
  ...
  // Prefer caller-supplied RTP timestamp; otherwise inherit the value the
  // WebRTC pipeline assigned to the dummy VideoFrame so the remote jitter
  // buffer sees a monotonically increasing timeline.
  ...
  // Mirror the timing fields hardware encoders set so the RTP sender +
  // remote jitter buffer see consistent timing for every frame.
...
void PassthroughVideoEncoder::SetRates(
  ...
  // Bitrate is controlled externally for passthrough -- treat 0 as "stop".
  sending_ = parameters.bitrate.get_sum_bps() != 0;
}
...
EncodedSourceRegistry::find_by_codec_name(
