...
RtcpFeedback to_capi_rtcp_feedback(webrtc::RtcpFeedback feedback) {
  RtcpFeedback capi{};
  capi.feedback_type = static_cast<RtcpFeedbackType>(feedback.type);

  if (feedback.message_type.has_value()) {
    capi.has_message_type = true;
    capi.message_type =
...
RtpCodecCapability to_capi_rtp_codec_capability(
  ...
  if (capability.clock_rate.has_value()) {
    ...
    capi.clock_rate = capability.clock_rate.value();
  }

  if (capability.preferred_payload_type.has_value()) {
    capi.has_preferred_payload_type = true;
    capi.preferred_payload_type = capability.preferred_payload_type.value();
  }

  if (capability.num_channels) {
    capi.has_num_channels = true;
    capi.num_channels = capability.num_channels.value();
  }

  for (auto feedback : capability.rtcp_feedback)
    capi.rtcp_feedback.push_back(to_capi_rtcp_feedback(feedback));

  for (auto param : capability.parameters)
    capi.parameters.push_back(StringKeyValue{param.first, param.second});
...
RtpHeaderExtensionCapability to_capi_rtp_header_extension_capability(
  ...
  RtpHeaderExtensionCapability capi{};
  capi.uri = header.uri;
  if (header.preferred_id.has_value()) {
    capi.has_preferred_id = true;
    capi.preferred_id = header.preferred_id.value();
  }
...
RtpExtension to_capi_rtp_extension(webrtc::RtpExtension ext) {
  ...
  capi.uri = ext.uri;
  capi.id = ext.id;
  capi.encrypt = ext.encrypt;
  return capi;
}

RtpFecParameters to_capi_rtp_fec_parameters(webrtc::RtpFecParameters fec) {
  RtpFecParameters capi{};
  if (fec.ssrc.has_value()) {
    capi.has_ssrc = true;
    capi.ssrc = fec.ssrc.value();
  }

  capi.mechanism = static_cast<FecMechanism>(capi.mechanism);
  return capi;
}

RtpRtxParameters to_capi_rtp_rtx_parameters(webrtc::RtpRtxParameters param) {
  RtpRtxParameters capi{};
  if (param.ssrc.has_value()) {
    capi.has_ssrc = param.ssrc.has_value();
    capi.ssrc = param.ssrc.value();
  }
  return capi;
}
...
RtpEncodingParameters to_capi_rtp_encoding_parameters(
  ...
  RtpEncodingParameters capi{};
  if (params.ssrc.has_value()) {
    capi.has_ssrc = params.ssrc.has_value();
    capi.ssrc = params.ssrc.value();
  }

  capi.bitrate_priority = params.bitrate_priority;
  capi.network_priority = static_cast<NetworkPriority>(params.network_priority);
  if (params.max_bitrate_bps.has_value()) {
    capi.has_max_bitrate_bps = true;
    capi.max_bitrate_bps = params.max_bitrate_bps.value();
  }

  if (params.min_bitrate_bps.has_value()) {
    capi.has_min_bitrate_bps = true;
    capi.min_bitrate_bps = params.min_bitrate_bps.value();
  }

  if (params.max_framerate.has_value()) {
    capi.has_max_framerate = true;
    capi.max_framerate = params.max_framerate.value();
  }

  if (params.num_temporal_layers.has_value()) {
    capi.has_num_temporal_layers = true;
    capi.num_temporal_layers = params.num_temporal_layers.value();
  }

  if (params.scale_resolution_down_by.has_value()) {
    capi.has_scale_resolution_down_by = true;
    capi.scale_resolution_down_by = params.scale_resolution_down_by.value();
  }

  if (params.scalability_mode.has_value()) {
    capi.has_scalability_mode = true;
    capi.scalability_mode = params.scalability_mode.value();
  }

  capi.active = params.active;
  capi.rid = params.rid;
  capi.adaptive_ptime = params.adaptive_ptime;
  return capi;
}
...
RtpCodecParameters to_capi_rtp_codec_parameters(
  ...
  if (params.clock_rate.has_value()) {
    capi.has_clock_rate = true;
    capi.clock_rate = params.clock_rate.value();
  }

  if (params.num_channels.has_value()) {
    capi.has_num_channels = true;
    capi.num_channels = params.num_channels.value();
  }
  ...
  for (auto pair : params.parameters)
    capi.parameters.push_back(StringKeyValue{pair.first, pair.second});

  return capi;
}

RtpCapabilities to_capi_rtp_capabilities(webrtc::RtpCapabilities capabilities) {
  RtpCapabilities capi{};
  for (auto codec : capabilities.codecs)
    capi.codecs.push_back(to_capi_rtp_codec_capability(codec));

  for (auto header : capabilities.header_extensions)
    capi.header_extensions.push_back(
        to_capi_rtp_header_extension_capability(header));

  for (auto fec : capabilities.fec)
    capi.fec.push_back(static_cast<FecMechanism>(fec));
...
RtcpParameters to_capi_rtcp_parameters(webrtc::RtcpParameters params) {
  ...
  capi.cname = params.cname;
  capi.reduced_size = params.reduced_size;
  capi.mux = params.mux;
  return capi;
}
...
RtpParameters to_capi_rtp_parameters(webrtc::RtpParameters params) {
  RtpParameters capi{};
  capi.transaction_id = params.transaction_id;
  capi.mid = params.mid;

  for (auto codec : params.codecs)
    capi.codecs.push_back(to_capi_rtp_codec_parameters(codec));

  for (auto header : params.header_extensions)
    capi.header_extensions.push_back(to_capi_rtp_extension(header));

  for (auto encoding : params.encodings)
    capi.encodings.push_back(to_capi_rtp_encoding_parameters(encoding));

  capi.rtcp = to_capi_rtcp_parameters(params.rtcp);

  if (params.degradation_preference.has_value()) {
    capi.has_degradation_preference = true;
    capi.degradation_preference = static_cast<DegradationPreference>(
